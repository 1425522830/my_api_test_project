...
2. **429限流自愈机制**：在底层封装了自动重试逻辑，检测到429限流后等待10秒自动重试，极大提高了测试稳定性。
3. **动态数据闭环**：通过 `new_discussion` 等 Fixture 实现自动造数、动态提取ID、测试结束自动清理，避免测试数据污染。
4. **安全数据隔离**：密码通过 `.env` 文件注入，不写入Git仓库。
5.**非标准 HTTP 方法适配与软删除**：此论坛项目的前端实际是通过 POST 结合 X-Http-Method-Override 或 PATCH 方式模拟删除/编辑，且删除并非物理删除，而是传 {"isHidden": true} 的软删除。通过抓包等方式，针对不同接口（删主贴用 PATCH、删评论用 PATCH）编写了精确的请求封装，并配合软删除机制成功实现了数据清理。

## 运行方式
1. 安装依赖：`pip install -r requirements.txt`
2. 配置 `.env` 文件放置账号密码的关键信息
